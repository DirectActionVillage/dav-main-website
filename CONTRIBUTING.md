...
Thanks for helping out with the Direct Action Village site. This is a static HTML and CSS site without a build step or shipped JavaScript. The `npm` setup below exists only for local dev tooling (formatting, linting, accessibility checks); none of it reaches the deployed site.
...
npm 12 and later block dependency install scripts unless they are allow-listed. `package.json` holds an `allowScripts` approval for puppeteer's postinstall (the browser download pa11y needs), pinned to the current version. After a puppeteer version bump, re-approve it with `npm install-scripts approve puppeteer`.
...
- `lint:prose`: runs the [house style](#house-style) checks on page copy and docs (`config/check-prose.mjs`)
- `test:a11y`: runs pa11y-ci accessibility tests
- `prepare`: installs Husky hooks (run automatically after `npm ci`)

Only `fix`, `format` and the two `:fix` scripts write to your files. Everything else only reports and exits. The pre-commit hook and CI need that from a check.
...
- The `.husky/commit-msg` hook runs the [house style](#house-style) checks on the commit message
...
OSV-Scanner reads the whole of `package-lock.json`, not the diff. If `main` has advisories in four packages and a PR fixes one of them, the Security job still fails on the other three. So the PR looks broken when it is not.

Merging them one at a time does not help either. Each merge rewrites the lockfile and leaves every other open PR stale. They all need a rebase and another CI cycle, and `main` stays red the whole way through.
...
Was it red before you touched it? Check `main` and compare:
...
When only Security is red, it is nearly always a tree-wide advisory rather than something the PR introduced. Read the OSV table in the log and consolidate as above.

A job canceled before any step ran isn't your code. A job that never got a runner reports an empty runner name and zero steps:
...
- The PR description includes the upstream release notes and commit list. Read them before trusting a major.
...
- Name branches `type/short-slug` after the squashed commit's type (for example `info/refactor-navbar`).
...
- Write each paragraph as one line rather than hard wrapping it. GitHub rewraps the body to fit the reader's window. A paragraph already broken at a fixed column comes out ragged there. `commit-msg` rejects a body that looks hard wrapped.
...
Write the description before you open the PR rather than after. GitHub keeps a public revision history for every edited PR body. Nothing short of deleting the repository clears it, so an edit is permanent and visible. For a real rewrite, replace the PR with a new one.
